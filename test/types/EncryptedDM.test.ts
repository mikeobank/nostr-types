...
test("encrypt, decrypt", async () => {
  ...
  const message = "hello"
  const encryptedMessage = await encrypt(webcrypto.subtle)(keyPairFrom.privateKey)(keyPairTo.publicKey, message)
  expect(encryptedMessage).not.toBe(message)
  const decryptedMessage = await decrypt(webcrypto.subtle)(keyPairTo.privateKey)(keyPairFrom.publicKey, encryptedMessage)
...
test("encryptEvent, decryptEvent", async () => {
  ...
  const encryptedDM = await encryptEvent(webcrypto.subtle)(privateKey)(keyPairTo.publicKey, event)
  ...
  const decryptedEvent = await decryptEvent(webcrypto.subtle)(keyPairTo.privateKey)(encryptedDM)
...
test("isEncryptedDM", async () => {
  ...
  const encryptedDM = await encryptEvent(webcrypto.subtle)(privateKey)(publicKey, event)
