...
test("encrypt, decrypt", async () => {
  ...
  const message = "hello"
  const encryptedMessage = await encrypt(webcrypto.subtle as SubtleCrypto)(keyPairFrom.privateKey)(keyPairTo.publicKey, message)
  expect(encryptedMessage).not.toBe(message)
  const decryptedMessage = await decrypt(webcrypto.subtle as SubtleCrypto)(keyPairTo.privateKey)(keyPairFrom.publicKey, encryptedMessage)
...
test("encryptEvent, decryptEvent", async () => {
  ...
  const encryptedDM = await encryptEvent(webcrypto.subtle as SubtleCrypto)(privateKey)(keyPairTo.publicKey, event)
  ...
  const decryptedEvent = await decryptEvent(webcrypto.subtle as SubtleCrypto)(keyPairTo.privateKey)(encryptedDM)
...
test("isEncryptedDM", async () => {
  ...
  const encryptedDM = await encryptEvent(webcrypto.subtle as SubtleCrypto)(privateKey)(publicKey, event)
